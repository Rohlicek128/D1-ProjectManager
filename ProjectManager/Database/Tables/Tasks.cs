...
public static class Tasks
{
    ...
    public static List<Task> List()
    {
        try
        {
            ...
            using var command = new SqlCommand(
                "select t.id, t.project_id, t.assigned_user_id, t.title, t.description, t.status, t.priority, t.completed from Tasks t",
                conn
            );
            ...
            while (reader.Read())
            {
                var project = Projects.FindById(reader.GetInt32(1)) ?? new Project("", "", DateTime.MinValue);
                var user = reader.IsDBNull(2) ? new User("", "", DateTime.MinValue, -1) : Users.FindById(reader.GetInt32(2)) ?? new User("", "", DateTime.MinValue, -1);
